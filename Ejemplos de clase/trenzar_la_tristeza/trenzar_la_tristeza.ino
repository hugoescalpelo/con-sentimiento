...
// -------------------------
const char* ssid     = "trenzar-la";
const char* password = "la-tristeza";
...
// -------------------------
// BOTONES (6 botones con INPUT_PULLUP)
// -------------------------
const uint8_t buttonPins[6] = {32, 33, 25, 26, 27, 14};  // ajusta si los necesitas en otros pines

int buttonState[6];           // estado estable (debounce)
int lastButtonReading[6];     // última lectura cruda
...
int frase_idx = 0;  // índice actual de frase OLED
...
void cambiarFraseSiguiente();    // NUEVO
void leerBotones();              // NUEVO
...
// -------------------------
// CAMBIAR FRASE (para los botones)
// -------------------------
void cambiarFraseSiguiente() {
  frase_idx = (frase_idx + 1) % total_frases_oled;
  strcpy(mensaje, frases_oled[frase_idx]);
  mensaje_actualizado = true;
...
void leerBotones() {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    // cambio en la lectura cruda => reset de debounce
    ...
    if ((now - lastDebounceTime[i]) > debounceDelay) {
      // si ha pasado el debounce, tomamos esto como estado estable
      if (reading != buttonState[i]) {
        buttonState[i] = reading;

        // transición a LOW = botón presionado (INPUT_PULLUP)
        if (buttonState[i] == LOW) {
          cambiarFraseSiguiente();
        }
      }
    }
  }
...
void setup() {
  ...
  // Mensaje inicial OLED (usamos frases_oled[0] para arrancar)
  frase_idx = 0;
  strcpy(mensaje, frases_oled[frase_idx]);
...
void loop() {
  ...
  // leer botones (cada pulsación avanza la frase)
  leerBotones();
